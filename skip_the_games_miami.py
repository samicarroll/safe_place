...
def run(selected_keywords):
    ...
    LIST = []
    url = 'https://skipthegames.com/posts/miami/'
    ...
    for urls in links:
        ...
        for keyword in selected_keywords:
            if keyword in description:
                ...
                screenshot_dir = pathlib.Path.home() / f"Desktop/skipthegames/miami/screenshots/{timestamp}"
    ...
    excel_dir = pathlib.Path.home() / f"Desktop/skipthegames/miami/excel_files/{timestamp}"
    if not os.path.exists(excel_dir):
        os.makedirs(excel_dir)
    df.to_excel(excel_dir / f"skipthegames({timestamps}).xlsx", index=False)
    print(f'skipthegamesftmyers({timestamps}).xlsx exported.')
    # CLOSE WEBDRIVER
    driver.close()
