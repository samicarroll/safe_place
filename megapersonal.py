...
def screen_shot(screenshot):
  # driver is your WebDriver
    driver = webdriver.Chrome()
    screenshot = driver.get_screenshot_as_file('screenshot.png')

    root_cws = os.getcwd()
    root_fold = os.path.abspath(root_cws)

    source_path = os.path.abspath(screenshot)
    source = source_path

    test_screenshots_dir = os.path.join(os.path.dirname(root_fold), 'test-screenshots')
    os.makedirs(test_screenshots_dir, exist_ok=True) # creates screenshot dir in cws root dir.
    test_screenshots = os.path.join(test_screenshots_dir, f"{screenshot}.png")
    new_dest = os.path.abspath(test_screenshots)

    shutil.copy(source, new_dest) # stores screenshot into the test-screenshot dir.
    print(f"Saving screenshot as: {screenshot} in path: {test_screenshots}")
